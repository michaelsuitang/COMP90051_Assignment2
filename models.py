...
class FcEncoder(nn.Module):
    def __init__(self, in_channel=3, input_dim=64*64, output_dim=256):
...
# TODO: Too many parameters now. Add more layers/MaxPool/FC layer at the end?
class CnnEncoder(nn.Module):
    def __init__(self, hidden_c1=8, hidden_c2=16, hidden_c3=64, latent_dim=256, kernel_size=9, padding=4):
        ...
        self.conv4 = nn.Conv2d(in_channels=hidden_c3, out_channels=latent_dim, kernel_size=kernel_size, padding=padding)
...
class DensenetEncoder(nn.Module):
    ...
    def forward(self, x):
        z = self.encoder(x)              # shape: (B, 1024, 2, 2)
...
class Decoder(nn.Module):
    def __init__(self, latent_dim=256):
        ...
        self.deconv4 = nn.ConvTranspose2d(8, 3, kernel_size=4, stride=2, padding=1)     # 32x32 -> 64x64

    def forward(self, z):
        x = self.fc(z)                      # [batch, 64*4*4]
